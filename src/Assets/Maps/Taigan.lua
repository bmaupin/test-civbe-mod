...
--	FILE:	 Taigan.lua
--	AUTHOR:  Bob Thomas
--	PURPOSE: Global map script - A cold world, with limited cropland.
------------------------------------------------------------------------------
...
function GetMapScriptInfo()
	...
	return {
		Name = "TXT_KEY_MAP_TAIGAN",
		Type = "TXT_KEY_MAP_TAIGAN",
		Description = "TXT_KEY_MAP_TAIGAN_HELP",
		IsAdvancedMap = 0,
		IconIndex = 15,
		SortIndex = 1,
...
function PangaeaFractalWorld:GeneratePlotTypes(args)
	...
	-- Now shift everything toward one of the poles.
...
function GeneratePlotTypes()
	print("Generating Plot Types (Lua Taigan) ...");
	
	-- Obtain landmass type selected by user.
	userInputLandmass = Map.GetCustomOption(4) -- GLOBAL variable
	if userInputLandmass == 5 then -- Random
		userInputLandmass = 1 + Map.Rand(6, "Arborea Random Landmass Type - Lua");
		if userInputLandmass > 4 then
			userInputLandmass = userInputLandmass - 4; -- Extra weighting for larger landmass types. 1/3 Pangaea, 1/3 Continents, 1/3 the two naval types combined.
		end
	end
	...
	elseif userInputLandmass == 4 then -- Islands
		local sea = Map.GetCustomOption(2)
		if sea == 4 then
...
function TerrainGenerator:InitFractals()
	self.snow = Fractal.Create(	self.iWidth, self.iHeight, 
									self.grain_amount, self.fractalFlags, 
									self.fracXExp, self.fracYExp);
									
	self.iSnow = self.snow:GetHeight(40);
...
function TerrainGenerator:GenerateTerrainAtPlot(iX,iY)
	...
	local terrainVal = self.terrainTundra;
	...
	elseif(lat <= 0.2) then
		...
		if plot:IsCoastalLand() then
			...
			local snowVal = self.snow:GetHeight(iX, iY);
			if (snowVal >= self.iSnow) then
				terrainVal = self.terrainSnow;
			end
		end
	end
...
function GenerateTerrain()
	print("Generating Terrain (Lua Taigan) ...");
...
------------------------------------------------------------------------------
function AddRivers()
	print("Generating Rivers, Canyons, and Lakes. (Lua Taigan) ...");

	local plots_per, min_pct = 9, 10;
	if userInputLandmass == 3 then
		plots_per, min_pct = 12, 5;
	elseif userInputLandmass == 4 then
		plots_per, min_pct = 14, 2;
	end
	local args = {plots_per_river_edge = plots_per, minimum_percentage_of_total_land = min_pct, };
...
function FeatureGenerator:__initFractals()
	...
	self.iWildAreaLevel = self.forestclumps:GetHeight(85)	-- Custom
	self.iWildAreaCore = self.forestclumps:GetHeight(85)	-- Custom
	self.iWildAreaPeriphery = self.forestclumps:GetHeight(70) -- Custom
...
function FeatureGenerator:__initTundraWildHeights()
	...
	-- No need for fancy checks. Taigain maps will always have enough tundra around. Using same values as for the forest wilds.
	self.iTundraCoreLevel = self.forestclumps:GetHeight(85)
	self.iTundraPeripheryLevel = self.forestclumps:GetHeight(70)
end
------------------------------------------------------------------------------
function FeatureGenerator:AddFeatures(allow_mountains_on_coast)
	...
	if allow_mountains_on_coast == false then -- Remove any mountains from coastal plots
		local num_mountains_softened = 0;
		for x = 0, self.iGridW - 1 do
			for y = 0, self.iGridH - 1 do
				local plot = Map.GetPlot(x, y)
				if plot:IsCoastalLand() then -- Check for mountain.
					if plot:GetPlotType() == PlotTypes.PLOT_MOUNTAIN then -- Soften mountain into hills.
						...
						num_mountains_softened = num_mountains_softened + 1;
					end
				end
			end
		end
		print("- -");
		print(num_mountains_softened, "mountains along coasts softened into hills.");
		print("- -");
	end
		
	-- Since Addtolls is a new method, older map scripts may not have it.
	-- Use default method in this case to prevent crashes.
	--
	-- No art support available for Atolls at release of BE, so disabling them for now. - Bob  July, 2014
	--if(self.AddAtolls ~= nil) then
		--self:AddAtolls(); -- Adds Atolls to oceanic maps.
	--else
		--FeatureGenerator.AddAtolls(self);
	--end
	
	self:AdjustTerrainTypes(); -- Softens arctic rivers. NOTE: Moved forward for Taigan.
...
function FeatureGenerator:AdjustTerrainTypes()
	-- Soften arctic terrain types at rivers.
		...
		for x = 0, width do
			...
			if (plot:IsRiver()) then
				...
				if (terrainType == self.terrainTundra) then
					plot:SetTerrainType(self.terrainGrass, false, true) -- Soften to Grass, not Plains, for Taigan.
...
function AddFeatures()
	print("Adding Features (Lua Taigan) ...");
...
function AssignStartingPlots:__CustomInit()
	self.ice_plots_next_to_warmer_plot = {};
	self.next_to_tundra_wild_plots = {};
end
